...
const MonthlyReportPage = ({ recordDate = "2026-03-24", dailyStatuses = {} }) => {
  // --- 데이터 연동 로직 ---
      ...
      <section>
        ...
        >
          <tr className="divide-x divide-slate-200 hover:bg-slate-50">
            ...
            <td className="px-2 py-1.5 text-right font-mono">{(composeData.reduce((s,i) => s + i.totalBalance, 0) - composeData.reduce((s,i) => s + (i.deposits - i.withdrawals), 0)).toLocaleString()}</td>
            <td className="px-2 py-1.5 text-right font-mono">{composeData.reduce((s,i) => s + i.deposits, 0).toLocaleString()}</td>
            <td className="px-2 py-1.5 text-right font-mono text-red-500">{composeData.reduce((s,i) => s + i.withdrawals, 0).toLocaleString()}</td>
            <td className="px-2 py-1.5 text-right font-mono font-bold">{composeData.reduce((s,i) => s + i.totalBalance, 0).toLocaleString()}</td>
            <td className="px-2 py-1.5"></td>
          </tr>
          <tr className="divide-x divide-slate-200 hover:bg-slate-50">
            <td className="px-2 py-1.5 font-bold">컴포즈커피스마트팩토리</td>
            <td className="px-2 py-1.5 text-right font-mono">{(smartData.reduce((s,i) => s + i.totalBalance, 0) - smartData.reduce((s,i) => s + (i.deposits - i.withdrawals), 0)).toLocaleString()}</td>
            <td className="px-2 py-1.5 text-right font-mono">{smartData.reduce((s,i) => s + i.deposits, 0).toLocaleString()}</td>
            <td className="px-2 py-1.5 text-right font-mono text-red-500">{smartData.reduce((s,i) => s + i.withdrawals, 0).toLocaleString()}</td>
            <td className="px-2 py-1.5 text-right font-mono font-bold">{smartData.reduce((s,i) => s + i.totalBalance, 0).toLocaleString()}</td>
            <td className="px-2 py-1.5"></td>
          </tr>
        </ReportTable>
      </section>
        ...
        {lastStatus ? (
            <>
                ...
                footer={
                    ...
                    <td className="px-2 py-1.5 text-right font-mono text-red-600 border border-red-500 bg-white">{composeData.reduce((s,i) => s + i.totalBalance, 0).toLocaleString()}</td>
                    <td className="px-2 py-1.5"></td>
                    </tr>
                }
                >
                ...
                footer={
                    <tr className="divide-x divide-slate-300">
                    <td colSpan={4} className="px-2 py-1.5 text-center font-bold">소 계</td>
                    <td className="px-2 py-1.5 text-right font-mono text-red-600 border border-red-500 bg-white">{smartData.reduce((s,i) => s + i.totalBalance, 0).toLocaleString()}</td>
                    <td className="px-2 py-1.5"></td>
                    </tr>
                }
                >
