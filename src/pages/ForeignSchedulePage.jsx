    ...
    reader.onload = async (evt) => {
      ...
      const parsedContracts = data.map(row => {
        ...
        return {
          ...
          invoiceWeight: getVal('인보이스중량') || '',
          actualUSD: getVal('실제송금액(USD)') || getVal('예상송금액(USD)') || '',
          actualKRW: getVal('실제송금액(KRW)') || getVal('예상송금액(KRW)') || '',
                  ...
                  {filteredSchedule.map(s => {
                    if (editingScheduleId === s.id) {
                      return (
                        <tr key={s.id} className="bg-indigo-50/50 transition-colors">
                          ...
                          <td className="px-4 py-2 border-r">
                            <input type="text" name="desc" value={editScheduleData.desc || ''} onChange={handleEditScheduleChange} className="w-full text-xs font-bold bg-white border border-slate-200 rounded px-2 py-1 outline-none" />
                          </td>
                          <td className="px-4 py-2 border-r">
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-slate-50 transition-all font-black text-slate-900">
                        ...
                        <td className="px-8 py-4 border-r text-[10px] text-slate-500 font-medium truncate max-w-[200px]">{s.desc}</td>
